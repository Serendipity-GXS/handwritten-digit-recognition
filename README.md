...
用 C11 从零实现 MNIST 手写数字识别，**不依赖任何深度学习框架**。张量运算、前向传播、反向传播、梯度下降全部手写，每一行代码都对应得上一个数学公式。

| 项目 | 说明 |
|---|---|
| 测试集准确率 | **97.19%**（9719 / 10000） |
| 网络结构 | 784 → 128(ReLU) → 64(ReLU) → 10(Softmax) |
| 参数量 | 109,386 |
| 训练耗时 | 约 50 秒（60000 样本 × 10 轮，gcc -O2 单线程） |
| 代码量 | 约 1100 行 C（含头文件，不含生成的权重） |
| 第三方依赖 | 无，仅 C 标准库 |

## 特性

- **零框架、零依赖**：Dense / ReLU / Softmax 的前向与反向、交叉熵损失、SGD 全部手写，方便逐层打断点验证
- **训练 / 推理双模式**：改一个宏即可切换，训练模式跑完自动导出权重，推理模式直接加载权重跳过训练
- **权重导出成 C 头文件**：`src/weights.h` 是纯 C 数组，既能回读复现，也能直接编译进嵌入式工程
- **错例可视化**：测试集认错的样本会打印字符画 + 10 类概率分布，直观看到模型错在哪、错得多离谱
- **结果可复现**：固定随机种子 + 固定权重导出精度（`%.9g` 保证 float 无损往返），相同配置每次跑出的结果一致

## 快速开始

### 环境要求

- GCC（Windows 下推荐 MinGW-w64；Linux / macOS 用系统自带 gcc 或 clang 均可）
- 支持 C11
...
如需重新获取（例如文件损坏），用附带脚本一键下载 + 解压 + 校验字节数：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/download_mnist.ps1
```

默认走 googleapis 镜像（国内速度较好），网络不通时可换源：
...
> `Makefile` 内的 `clean` 目标使用了 Windows 的 `del` 命令，Linux 下改用上面的 gcc 一行命令即可。
...
> 程序用相对路径读写 `data/` 和 `src/weights.h`，**请在项目根目录下运行**。
...
推理模式不再训练，直接加载 `src/weights.h` 后跑测试集，秒级完成。
...
这个错例挺典型：数字下半部分有个闭合的圈，模型把它当成了 4（0.51）而不是 2（0.03）——但它其实也很犹豫（9 拿到 0.41）。**全连接网络把图片压成一维向量，丢掉了"笔画在哪个位置"的空间信息**，所以对这种结构上的歧义没什么抵抗力。这正是下一步要上卷积网络（CNN）的原因。
...
├── Makefile                    # mingw32-make 构建（Windows / MinGW）
├── README.md                   # 本文档
├── 调参指南.txt                # 可调参数清单 + 各参数的影响和实验现象
├── 手写数字识别调参指南.pdf     # 上面那份的 PDF 版
├── docs/                       # README 配图（网络结构、前向传播示意图）
├── data/                       # MNIST 数据集（4 个 idx 文件，已随仓库提供）
├── weights/                    # 预留：后续二进制模型文件目录
├── scripts/
│   └── download_mnist.ps1      # 数据集下载 / 解压 / 校验
└── src/
    ├── main.c                  # 入口：训练 / 推理两种模式 + 测试集评估 + 错例可视化
...
|---|---|---|
| 0 | 环境搭建 + 数据集获取 | ✅ 完成 |
| 1 | 数据层：idx 读取、归一化、字符画 | ✅ 完成 |
| 2 | 张量工具层 | ✅ 完成 |
| 3 | MLP 热身网络（全连接 + 反向传播） | ✅ 完成，测试集 97.19% |
| 4 | **CNN**：卷积 / 池化及其反向传播、特征图可视化 | 🚧 进行中 |
| 5 | 工程化：二进制模型文件、混淆矩阵、图片识别 demo | 📋 计划中 |
| 6 | 性能优化 / 嵌入式延伸（im2col、定点量化、移植 MCU） | 💡 可选 |

## 文档

- **[调参指南.txt](调参指南.txt)** —— 调参手册：8 个可调参数的位置（文件 + 行号）与预期效果、终端输出里各个单词的含义、若干调参实验方向。另有 [PDF 版](手写数字识别调参指南.pdf)

## 已知限制

- 当前是纯全连接网络，未利用图像的二维空间结构（CNN 开发中）
- 单样本 SGD，尚未实现 mini-batch / momentum
- 卷积层尚未做 `im2col`、多线程等性能优化（计划在阶段 6）
- Makefile 面向 Windows + MinGW，Linux 下请用 gcc 一行命令
